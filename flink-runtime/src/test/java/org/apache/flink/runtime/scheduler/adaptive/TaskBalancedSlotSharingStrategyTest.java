...
import org.apache.flink.runtime.scheduler.adaptive.allocator.TaskBalancedSlotSharingStrategy;
...
/**
 * Test for {@link
 * org.apache.flink.runtime.scheduler.adaptive.allocator.TaskBalancedSlotSharingStrategy}.
 */
class TaskBalancedSlotSharingStrategyTest {
    ...
    void testGetExecutionSlotSharingGroupsInOneSlotSharingGroup() {
        ...
        final JobInformation.VertexInformation vertex3 =
                new TestVertexInformation(new JobVertexID(), 3, slotSharingGroup1);
        ...
        final List<SlotSharingSlotAllocator.ExecutionSlotSharingGroup> executionSlotSharingGroups =
                        ...
                        executionSlotSharingGroups.stream()
                                .map(
                                        SlotSharingSlotAllocator.ExecutionSlotSharingGroup
                                                ::getSlotSharingGroup))
                .containsExactly(slotSharingGroup1);
        ...
        assertThat(executionSlotSharingGroups)
                .allSatisfy(
                        executionSlotSharingGroup -> {
                            assertThat(executionSlotSharingGroup.getContainedExecutionVertices())
                                    .hasSize(2);
                            assertThat(
                                            executionSlotSharingGroup
                                                    .getContainedExecutionVertices().stream()
                                                    .map(ExecutionVertexID::getJobVertexId)
                                                    .collect(Collectors.toList()))
                                    .contains(vertex3.getJobVertexID());
                        });
    }
    ...
    void testGetExecutionSlotSharingGroupsInOneSlotSharingGroupWithCoLocationGroup() {
        final JobInformation.VertexInformation vertex1 =
                new TestVertexInformation(
                        new JobVertexID(), 1, slotSharingGroup1, coLocationGroup1);
        final JobInformation.VertexInformation vertex2 =
                new TestVertexInformation(
                        new JobVertexID(), 2, slotSharingGroup1, coLocationGroup1);
        final JobInformation.VertexInformation vertex3 =
                new TestVertexInformation(new JobVertexID(), 3, slotSharingGroup1);
        final TestJobInformation testJobInformation =
                new TestJobInformation(Arrays.asList(vertex1, vertex2, vertex3));
        ...
        assertThat(
                        executionSlotSharingGroups.stream()
                                .map(group -> group.getContainedExecutionVertices().size())
                                .sorted())
    ...
    void testGetExecutionSlotSharingGroupsInMultiSlotSharingGroups() {
        ...
        final JobInformation.VertexInformation vertex4 =
                new TestVertexInformation(new JobVertexID(), 1, slotSharingGroup2);
        final JobInformation.VertexInformation vertex5 =
                new TestVertexInformation(new JobVertexID(), 2, slotSharingGroup2);
                        ...
                        executionSlotSharingGroups.stream()
                                .map(
                                        SlotSharingSlotAllocator.ExecutionSlotSharingGroup
                                                ::getSlotSharingGroup)
                                .collect(Collectors.toSet()))
                .containsExactlyInAnyOrder(slotSharingGroup1, slotSharingGroup2);

        assertThat(
                        executionSlotSharingGroups.stream()
                                .filter(
                                        essg ->
                                                essg.getSlotSharingGroup()
                                                        .equals(slotSharingGroup1))
                                .collect(Collectors.toSet()))
                .allSatisfy(
                        executionSlotSharingGroup ->
                                assertThat(
    ...
    private VertexParallelism getVertexParallelism(
        ...
        return slotAllocator.determineParallelism(jobInformation, slots).get();
    }
}
