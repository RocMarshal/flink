...
/**
 * Class for rescale handlers that will cache the {@link RescalesStatsSnapshot} object.
 *
 * @param <R> the response type
 * @param <M> the message parameters
 */
@Internal
public abstract class AbstractJobRescalesHandler<
    ...
    private final Cache<JobID, CompletableFuture<RescalesStatsSnapshot>>
            jobRescaleStatsSnapshotCache;

    protected AbstractJobRescalesHandler(
        ...
        this.jobRescaleStatsSnapshotCache = jobRescaleStatsSnapshotCache;
    }
    ...
    protected CompletableFuture<R> handleRequest(
        ...
        try {
            return jobRescaleStatsSnapshotCache
                    .get(jobId, () -> gateway.requestRescalesStats(jobId, timeout))
                    .thenApplyAsync(
                            rescalesStatsSnapshot -> {
                                try {
                                    return handleRescalesStatsRequest(
                                            request, rescalesStatsSnapshot);
    ...
    protected abstract R handleRescalesStatsRequest(
