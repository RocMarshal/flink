...
public class JobRescaleStatisticsDetailsHandler
        extends AbstractExecutionGraphHandler<
                JobRescaleStatisticsDetails, JobIDRescaleIDParameters> {
    ...
    @Override
    protected JobRescaleStatisticsDetails handleRequest(
        ...
        AbstractID rescaleUuid = request.getPathParameter(JobRescaleIDPathParameter.class);
        Rescale rescale = rescalesStatsSnapshot.getRescale(rescaleUuid);

        if (rescale == null) {
            throw new RestHandlerException(
                    "Could not find rescale statistics for rescale " + rescaleUuid + '.',
                    HttpResponseStatus.NOT_FOUND);
        }
