...
public class DefaultSlotAssigner implements SlotAssigner {
    ...
    public static final String DEPLOYMENT_TARGET = "embedded";

    private final @Nullable String deploymentTarget;

    public DefaultSlotAssigner(@Nullable String deploymentTarget) {
    ...
    public Collection<SlotAssignment> assignSlots(
        ...
        final Collection<? extends SlotInfo> pickedSlots =
                pickSlotsIfNeeded(allExecutionSlotSharingGroups.size(), freeSlots);
    ...
    private Collection<? extends SlotInfo> pickSlotsIfNeeded(
        ...
        if (DEPLOYMENT_TARGET.equalsIgnoreCase(deploymentTarget)
