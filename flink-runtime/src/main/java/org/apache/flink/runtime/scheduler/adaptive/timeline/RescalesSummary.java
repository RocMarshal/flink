...
public class RescalesSummary {
    ...
    public void addSealed(Rescale rescale) {
        if (!Rescale.isSealed(rescale)) {
            LOG.warn(
        ...
        if (pendingRescalesCount > 0) {
            pendingRescalesCount--;
        }

        switch (rescale.getStatus()) {
            case FAILED:
    ...
    public void addPending(Rescale rescale) {
        if (!Rescale.isSealed(rescale)) {
            LOG.warn("Unexpected rescale: {}, which will be ignored.", rescale);
        } else {
            if (pendingRescalesCount < 1) {
                pendingRescalesCount++;
            }
