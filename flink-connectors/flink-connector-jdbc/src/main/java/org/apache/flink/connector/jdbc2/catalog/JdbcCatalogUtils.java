...
public class JdbcCatalogUtils {
    ...
    public static AbstractJdbcCatalog createCatalog(
            CatalogFactory.Context context, ReadableConfig config) {
        JdbcDialect dialect = JdbcDialectLoader.load(config.get(BASE_URL));

        if (dialect instanceof PostgresDialect) {
            return new PostgresCatalog(context, config);
        } else if (dialect instanceof MySqlDialect) {
            return new MySqlCatalog(context, config);
        } else {
