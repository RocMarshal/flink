...
public class WordCount {
    ...
    public static void main(String[] args) throws Exception {
        ...
        config.set(TaskManagerOptions.MINI_CLUSTER_NUM_TASK_MANAGERS, 4);
        ...
        SingleOutputStreamOperator<String> ss =
                env.addSource(
                        ...
                        .map(s -> s)
                        .name("map1");
        SingleOutputStreamOperator<String> map11 = ss.map(s -> s).name("map2");
        ss.forward().map(s -> s).name("map3").disableChaining();

        map11.forward().map(s -> s).name("map4").disableChaining();
        map11.forward().map(s -> s).name("map5").disableChaining();
