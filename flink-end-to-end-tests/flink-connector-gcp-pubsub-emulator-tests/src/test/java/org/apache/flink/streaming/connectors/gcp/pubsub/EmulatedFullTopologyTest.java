...
class EmulatedFullTopologyTest extends GCloudUnitTestBase {
    ...
    void testFullTopology() throws Exception {
        ...
        for (String s : messagesToSend) {
            publisher
                    .publish(PubsubMessage.newBuilder().setData(ByteString.copyFromUtf8(s)).build())
                    .get();
        }
        publisher.shutdown();
        ...
        env
                ...
                .addSource(
                        // We create a PubSubSource which is the same as the normal source but with
                        // a self termination feature.
