...
class ExecuteAlterAddColumnComponentsSqlTest(isStreaming: Boolean,
  ...
  def testAddWMForNonexistField(): Unit = {
    ...
    processSqlForErrorCases("alter table orders add %s watermark for non_f " +
      "as non_f - interval '1' second %s")
  }
  ...
  def testAddMetaColForView(): Unit = {
    ...
    _expectedEx.expectMessage("ALTER TABLE for a view is not allowed")
  ...
  def testAddMetaColDuplicated(): Unit = {
    val sql = "alter table orders add %s amount timestamp_ltz(3) not null " +
      "metadata from 'timestamp' virtual comment 'mc_cmt' %s"
    _expectedEx
      .expectMessage("A column named 'amount' already exists in the base table")
    processSqlForErrorCases(sql)
  }
  ...
  def testAddComputedColForView(): Unit = {
    ...
    _expectedEx.expectMessage("ALTER TABLE for a view is not allowed")
    processSqlForErrorCases(sqlPat, JStr.format(sql, "orders", "", "cost int not null", "", ""))
  }
  ...
  def testAddComputedColDuplicated(): Unit = {
    ...
    _expectedEx.expectMessage("A column named 'amount' already exists in the base table")
    processSqlForErrorCases(sqlPat, JStr.format(sql, "orders", "", "cost int not null", "", ""))
  }
  ...
  def testAddComputedColUnknown(): Unit = {
    ...
    _expectedEx.expectMessage("Unknown identifier 'non_col'")
    processSqlForErrorCases(sqlPat, JStr.format(sql, "orders", "", "cost int not null", "", ""))
  }
  ...
  def testAddDuplicatedPhyCol(): Unit = {
    ...
    _expectedEx.expect(classOf[ValidationException])
    processSqlForErrorCases(sqlPat)
  }
  ...
  def testAddPhyColNotFoundReference(): Unit = {
    ...
    _expectedEx.expect(classOf[ReferencedColumnNotFoundException])
    processSqlForErrorCases(sqlPat)
  }
